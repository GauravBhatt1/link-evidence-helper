...
<body>
  ...
  <script>
    ...
    async function renderCollection(kind) {
      ...
      libraryViewEl.innerHTML=libraryToolbar(kind==="movie"?"Movies":"TV Shows",stats)+`<div class="library-filters"><input id="librarySearch" placeholder="Search ${kind==="movie"?"movies":"shows"}"><select id="libraryStatus"><option value="">All statuses</option><option value="available">Available</option><option value="missing">Missing</option><option value="unmatched">Needs Match</option></select><select id="librarySort"><option value="title">Title</option><option value="year">Year</option><option value="dateAdded">Recently added</option><option value="fileSize">File size</option><option value="rating">Rating</option></select></div><div class="library-grid" id="libraryGrid"><div class="library-empty">Loading library…</div></div>`;
    ...
    renderSources=async function(){try{const body=await api("/api/admin/sources"),rows=body.sources||[];
      const card=row=>`<div class="admin-row"><strong>${escapeHtml(row.name)}</strong><small>Domain: ${escapeHtml(row.domain)} · ${row.enabled?"Enabled":"Disabled"}<br>Search: ${escapeHtml(row.search_status)} · Link resolver: ${escapeHtml(row.resolver_status)}<br>Last test: ${escapeHtml(row.last_tested_at?displayTime(row.last_tested_at):"Never")}</small><span class="episode-row-actions"><button class="btn secondary source-test" data-id="${escapeHtml(row.id)}" data-kind="${escapeHtml(row.kind)}">Test</button><button class="btn secondary source-edit" data-id="${escapeHtml(row.id)}" data-kind="${escapeHtml(row.kind)}">Edit</button><button class="btn secondary source-toggle" data-id="${escapeHtml(row.id)}" data-kind="${escapeHtml(row.kind)}" data-enabled="${row.enabled?"0":"1"}">${row.enabled?"Disable":"Enable"}</button><button class="btn secondary source-advanced" data-id="${escapeHtml(row.id)}" data-kind="${escapeHtml(row.kind)}">Advanced Diagnostics</button><button class="btn secondary source-delete" data-id="${escapeHtml(row.id)}" data-kind="${escapeHtml(row.kind)}">Delete</button></span></div>`;
      libraryViewEl.innerHTML=`<div class="library-toolbar"><h2>Sources</h2><button class="btn" id="addSource">Add New Source</button></div><div class="pending-note">Configured sources and saved adapters are managed together.</div><div class="admin-list">${rows.map(card).join("")||"<div class='pending-note'>No sources configured.</div>"}</div><section class="admin-panel" id="sourceWizard" hidden></section>`;
