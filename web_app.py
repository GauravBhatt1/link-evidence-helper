...
<body>
  ...
  <script>
    ...
    const libraryState = { view: "search", page: 1, loading: false, scroll: {}, stats: null, detail: null, renderId: 0, controller: null };
    ...
    function libraryRoute(view) { const prefix=basePrefix()||"/index"; return { search:`${prefix}/`, movies:`${prefix}/library/movies`, tv:`${prefix}/library/tv`, missing:`${prefix}/library/missing`, recent:`${prefix}/library/recent`, admin:`${prefix}/admin${adminState?.section==="sources"?"/sources":""}` }[view] || `${prefix}/`; }
    ...
    function beginLibraryRender() {
      libraryState.controller?.abort();
      const controller=new AbortController(), id=++libraryState.renderId;
      libraryState.controller=controller;
      return {id,signal:controller.signal};
    }
    const libraryRenderIsCurrent=request=>Boolean(request)&&libraryState.renderId===request.id&&!request.signal.aborted;
    function navigate(view, push=true) {
      ...
      if(library) return renderLibrary(beginLibraryRender());
      libraryState.controller?.abort(); libraryState.controller=null; ++libraryState.renderId;
    ...
    async function renderCollection(kind, request) {
      ...
      if(!libraryRenderIsCurrent(request)) return;
      ...
      let loadController=null,loadId=0;
      const load=async(page=1)=>{ loadController?.abort(); const controller=new AbortController(),id=++loadId;loadController=controller; const search=$("librarySearch")?.value||"", status=$("libraryStatus")?.value||"", sort=$("librarySort")?.value||"title"; const result=await api(`${endpoint}?page=${page}&limit=30&search=${encodeURIComponent(search)}&status=${status}&sort=${sort}`,{signal:controller.signal}); if(!libraryRenderIsCurrent(request)||id!==loadId) return; const grid=$("libraryGrid"); if(!grid)return; grid.innerHTML=result.items.length?result.items.map(i=>card(i,kind)).join(""):'<div class="library-empty">No items yet. Run Scan Library after configuring the media folders.</div>'; if(result.page<result.totalPages) grid.insertAdjacentHTML("beforeend",`<button class="btn secondary" id="loadMore">Load more (${result.total-result.page*result.limit} remaining)</button>`); grid.querySelectorAll(".library-card").forEach(b=>b.onclick=()=>openLibraryDetail(b.dataset.kind,b.dataset.item)); $("loadMore")?.addEventListener("click",()=>load(result.page+1)); };
    ...
    async function renderMissing(request) { const stats=await libraryStats(request); if(!libraryRenderIsCurrent(request))return; libraryViewEl.innerHTML=libraryToolbar("Missing",{count:0,available:0,totalSize:0},false)+`<div class="library-filters"><select id="missingKind"><option value="movies">Missing Movies</option><option value="unmatched">Unmatched Files</option></select></div><div class="library-grid" id="libraryGrid"><div class="library-empty">Loading…</div></div>`; let loadId=0;const load=async()=>{const id=++loadId;const result=await api(`/api/library/missing?type=${$("missingKind").value}`,{signal:request.signal});if(!libraryRenderIsCurrent(request)||id!==loadId)return;const grid=$("libraryGrid");if(!grid)return;grid.innerHTML=result.items.length?result.items.map(i=>card(i,i.type)).join(""):'<div class="library-empty">Nothing to show.</div>'; grid.querySelectorAll(".library-card").forEach(b=>b.onclick=()=>openLibraryDetail(b.dataset.kind,b.dataset.item));}; $("missingKind").onchange=()=>load().catch(e=>{if(!requestWasCancelled(e))libraryError(e)}); load().catch(e=>{if(!requestWasCancelled(e))libraryError(e)}); }
    async function renderRecent(request) { const stats=await libraryStats(request); if(!libraryRenderIsCurrent(request))return; libraryViewEl.innerHTML=libraryToolbar("Recently Added",{count:0,available:0,totalSize:0},false)+`<div class="library-grid" id="libraryGrid"><div class="library-empty">Loading…</div></div>`; try {const result=await api("/api/library/recent?limit=40",{signal:request.signal});if(!libraryRenderIsCurrent(request))return;const grid=$("libraryGrid");if(!grid)return;grid.innerHTML=result.items.length?result.items.map(i=>card(i,i.type)).join(""):'<div class="library-empty">No recently detected files.</div>'; grid.querySelectorAll(".library-card").forEach(b=>b.onclick=()=>openLibraryDetail(b.dataset.kind,b.dataset.item));}catch(e){if(!requestWasCancelled(e)&&libraryRenderIsCurrent(request))libraryError(e)} }
    ...
    function bindAdminTabs() { libraryViewEl.querySelectorAll("[data-admin-section]").forEach(button=>button.onclick=()=>{adminState.section=button.dataset.adminSection;history.replaceState({view:"admin"},"",`${basePrefix()||"/index"}/admin${adminState.section==="sources"?"/sources":""}`);renderAdminWorkspace();}); }
    ...
    async function renderAdminWorkspace() {
      ...
      await renderAdminOverview(); libraryViewEl.insertAdjacentHTML("afterbegin",adminTabs()+`<div class="library-toolbar"><span class="library-stat">Admin unlocked</span><button class="btn secondary" id="lockAdmin">Lock Admin</button></div>`); bindAdminTabs(); $("lockAdmin").onclick=()=>{adminPassword="";adminState.unlocked=false;renderAdminWorkspace();};
    }
    ...
    async function renderLibrary(request=beginLibraryRender()) {
    ...
    document.addEventListener("pointerdown",event=>{const menu=$("mobileMoreMenu"),button=$("mobileMoreBtn");if(menu&&!menu.hidden&&!menu.contains(event.target)&&!button?.contains(event.target))closeMobileMore();});
